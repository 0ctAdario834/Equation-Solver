...
def create_sqrt_tab(notebook, window):
    tab_sqrteqnslvr = Frame(notebook)
    notebook.add(tab_sqrteqnslvr,text=" | Square Root Equation | ")
    tab_sqrteqnslvr.config(background='#999999')

    entry_accuracy = None

    name = Label(tab_sqrteqnslvr,
                 text="Square Root Equation Solver",
    ...
    accuracy_frame = Frame(tab_sqrteqnslvr, bg="#999999", relief=RAISED, border=16, padx=16, pady=8)
    ...
    coeff_frame = Frame(tab_sqrteqnslvr, bg="#999999", relief=RAISED, border=16, padx=16, pady=8)
    ...
    def expected_eqn():
        ...
        ax.axis('off')
        ax.text(0.5, 0.5, r"$y = a\sqrt{b(x - h)} + k$", fontsize=12, ha='center', va='center')
    ...
    def SolveSqrtEquation():
        ...
        try:
            if a == 0 or b == 0:
                messagebox.showerror("Error", "This is not a square root equation.")
                return
    ...
    solve_button = Button(tab_sqrteqnslvr,
                          text="Solve Equation",
                          command=SolveSqrtEquation,
                          border=8,
                          relief=RAISED,
                          padx=4,
                          pady=4)
    ...
    buttons_frame = Frame(tab_sqrteqnslvr, bg="#999999")
    ...
    results_frame = Frame(tab_sqrteqnslvr, bg="#999999", relief=RAISED, border=8, padx=8, pady=8)
